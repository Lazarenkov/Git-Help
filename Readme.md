...
> 4. [Работа с содержимым репозитория](#Работа-с-содержимым-репозитория)
...
#### Пулл из ремоут репозитория в другой локальный
> Если нужно сделать своего рода git clone поверх уже существующего локального репозитория, подтянув различия из ремоут, нужно выполнить следующей порядок действий:
* иницилизировать репозиторий в локальной папке куда будем пуллить `git init`
* добавить вручную .gitignore из ремоута `git add .gitignore`
* привязать ремоут `git remote add`
* закоммитить `git commit`
* сделать пулл из ремоута, разрешив нетрекать историю  `git pull ... --allow-unrelated-histories`
* [вручную отмерджить конфликты](#Решение-конфликтов-при-слиянии-веток)
* [закоммитить мердж](#Решение-конфликтов-при-слиянии-веток)
...
* `git rm file` - удалить фаил *file* из репозитория. Следом надо закоммитить. Адрес до файла должен начинаться с `./` чтобы гит шел к нему прямо из рут-папки репозитория. Например: `./src/test/java/Test.java`
* `git rm -r folder/` - удалить папку *folder* и ее содержимое из репозитория. Следом надо закоммитить 

#### Слияние репозиториев
- `git subtree add -P RepoA https://RepoB-url master` - команда выполнит добавление репозитория RepoB в репозиторий RepoA, создаст внутри него отдельную папку "RepoB", соответствующую ветку, со всеми коммитами из мастера RepoB, и выполнит мердж этой ветки с мастер веткой RepoA. Команду необходимо запускать находясь внутри RepoA.
---
